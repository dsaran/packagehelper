...
class FileTree(Delegate):
    ...
    def get_data(self):
        ...
        #items = [item[0] for item in self.fileTree.get_model()]
        #print "items: ", items
        #for item in items:
        #    print "-------------> ", self.fileTree.get_descendants(item)
        #    item.content = self.fileTree.get_descendants(item)
    ...
    #def on_fileTree__row_expanded(self, treeview, item):
    #    self._clean()
