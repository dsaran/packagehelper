...
class Gadget(gobject.GObject):
    ...
    def __init__(self, adaptor, project):
        ...
        # The widget associated with this gadget
        self.widget = None

        # Constructor, used by UIManager
    ...
    def copy(self, project=None, keep_name=False):
        ...
        if not keep_name:
            project.set_new_widget_name(self.widget)
        return gadget
    ...
    def setup_widget(self, widget):
        ...
        if self.widget.flags() & gtk.TOPLEVEL:
            self.widget.connect('delete-event', self._hide_on_delete)

        self.widget.connect('popup-menu', self._popup_menu)
    ...
    def _key_press(self, widget, event):
        ...
        if event.keyval in (gtk.keysyms.Delete, gtk.keysyms.KP_Delete):
            ...
            if len(self.project.selection) == 1:
                command_manager.delete_selection(gadget.project)
            return True
